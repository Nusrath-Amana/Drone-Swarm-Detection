...
class MoEModel(torch.nn.Module):
    ...
    def _apply(self, fn):
        ...
        self = super()._apply(fn)
        m = self.model[-1]  # Detect()
        if isinstance(m, MoEDetect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            m.stride = fn(m.stride)
            m.anchors = fn(m.anchors)
            m.strides = fn(m.strides)
        return self
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        print(">>>>> DetectionModel is being initialized")
        super().__init__()
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in frozenset({Detect,MoEDetect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}):
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect,MoEDetect, Segment, Pose, OBB}:
                m.legacy = legacy
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect,MoEDetect, WorldDetect, v10Detect)):
                return "detect"
